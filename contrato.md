...
#### 5. Obtener Comentarios de una Sugerencia

Permite recuperar la lista cronológica ascendente de comentarios comunitarios asociados a una propuesta o sugerencia específica, incluyendo los datos públicos del autor (nombre, foto de perfil, rol).

- **Método:** `GET`
- **Ruta:** `/sugerencias/:id/comentarios`
- **Parámetros de Ruta (Path Params):**
  * `id` (UUID, requerido): Identificador único de la sugerencia.
- **Encabezados opcionales:**
  * `x-user-role`: Rol del usuario consultante (ej. `alumno`, `profesor`, `admin`, `secretaria`).

##### 📤 Respuestas de Éxito (Status 200 OK)

Si la consulta es exitosa o si la sugerencia aún no tiene comentarios registrados:
...
```json
{
  ...
  "comentarios": [
    {
      "id": "176e9fed-ee59-4a2f-bc10-c215dea5e03f",
      "sugerencia_id": "d798a3e4-8cf1-4509-bc01-e24df234a9f9",
      "usuario_id": "60685e1f-3d41-42c2-b9a6-d71739856b22",
      "texto": "Excelente iniciativa para mejorar las instalaciones del colegio.",
      "created_at": "2026-08-27T18:15:29.826Z",
      "usuarios": {
        "id": "60685e1f-3d41-42c2-b9a6-d71739856b22",
...
En caso de indisponibilidad temporal de la base de datos o fallos no críticos, el endpoint retorna un `Status 200 OK` con un array vacío (`comentarios: []`) para evitar bloqueos en el frontend.

---

#### 6. Publicar un Comentario en una Sugerencia

Permite a un miembro autenticado de la comunidad educativa publicar un aporte o comentario sobre una propuesta existente. Aplica validación estricta de contenido mediante filtro institucional de lenguaje inapropiado.

- **Método:** `POST`
- **Ruta:** `/sugerencias/:id/comentarios`
- **Parámetros de Ruta (Path Params):**
  * `id` (UUID, requerido): Identificador único de la sugerencia a comentar.
- **Encabezados requeridos:**
  * `Content-Type: application/json`
- **Encabezados opcionales:**
  * `x-user-role`: Rol del autor del comentario.
...
{
  "usuario_id": "60685e1f-3d41-42c2-b9a6-d71739856b22",
  "texto": "Totalmente de acuerdo con esta propuesta, beneficiará a todo el curso."
}
```

##### 📤 Respuestas de Éxito (Status 201 Created)

Si el comentario pasa las validaciones y se registra correctamente en Supabase:
...
###### Error 400 - Bad Request (Faltan campos obligatorios)
Se devuelve cuando `usuario_id` o `texto` están ausentes o vacíos:
...
###### Error 400 - Bad Request (Lenguaje inapropiado detectado)
Se devuelve cuando el texto contiene palabras ofensivas o no permitidas:
...
###### Error 404 - Not Found (Sugerencia inexistente)
Se devuelve cuando el UUID en la ruta no corresponde a ninguna sugerencia:
